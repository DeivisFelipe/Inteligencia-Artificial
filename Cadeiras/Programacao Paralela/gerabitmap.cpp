...
#pragma pack(push, 1) // Garante que não haja padding
...
int main() {
    std::string inputImage = "imagens/entrada.bmp";
    std::string outputImage = "saida.bmp";

    // Read an image from disk, modify it and write it back:
    BMP bmp(inputImage.c_str());

    // Fill a region with a specific color
    bmp.fill_region(10, 10, 100, 100, 255, 0, 0, 255);

    // Desenha um retangulo vermelho no cantor superior esquerdo
    bmp.draw_rectangle(200, 200, 100, 100, 255, 0, 0, 255, 1);

    bmp.write(outputImage.c_str());
